...
import { TMemberDepartmentSchema } from '@/validations/member/member-department-schema'

import {
    useCreateMemberDepartment,
    useUpdateMemberDepartment,
} from '@/hooks/api-hooks/member/use-member-department'
...
export type TMemberDepartmentFormValues = z.infer<
    typeof TMemberDepartmentSchema
>
...
const MemberDepartmentCreateUpdateForm = ({
    ...
    const form = useForm<TMemberDepartmentFormValues>({
        resolver: zodResolver(TMemberDepartmentSchema),
    ...
    const createMutation = useCreateMemberDepartment({ onSuccess, onError })
    const updateMutation = useUpdateMemberDepartment({ onSuccess, onError })

    const onSubmit = form.handleSubmit((formData) => {
        if (memberDepartmentId) {
            updateMutation.mutate({ memberDepartmentId, data: formData })
        } else {
            createMutation.mutate(formData)
        }
    })

    const { error, isPending, reset } = memberDepartmentId
        ? updateMutation
        : createMutation
