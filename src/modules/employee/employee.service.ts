...
const getAllEmployee = async <TData = IUserOrganization<IEmployee>>({
    url,
    query,
}: {
    url?: string
    ...
    const newUrl = qs.stringifyUrl({
        url: url || `${userOrganizationAPIRoute}/employee`,
        query,
    })
...
export type TEmployeesFetchMode = 'employee' | 'owner'
...
export const useGetAllEmployees = ({
    ...
    mode?: TEmployeesFetchMode
    query?: TAPIQueryOptions
    options?: HookQueryOptions<IEmployee[], Error>
}) =>
    useQuery<IEmployee[], Error>({
        ...
        queryFn: () => {
            const url = `${userOrganizationAPIRoute}/${mode}/search`
            return getAllEmployee({ query, url })
        },
    })
