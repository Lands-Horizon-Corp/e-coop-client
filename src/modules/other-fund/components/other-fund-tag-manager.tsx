import { ReactNode, useMemo } from 'react'

import { toast } from 'sonner'

import { cn } from '@/helpers'
import { dateAgo, toReadableDateTime } from '@/helpers/date-utils'
import { serverRequestErrExtractor } from '@/helpers/error-message-extractor'
import {
    IOtherFundTag,
    useCreateOtherFundTag,
    useDeleteOtherFundTagById,
    useGetAllOtherFundTag,
} from '@/modules/other-fund-tag'
import TagTemplatePicker from '@/modules/tag-template/components/tag-template-picker'
import useConfirmModalStore from '@/store/confirm-modal-store'
import { type VariantProps, cva } from 'class-variance-authority'

import { PlusIcon, RenderIcon, TIcon, TagIcon, XIcon } from '@/components/icons'
import LoadingSpinner from '@/components/spinners/loading-spinner'
import InfoTooltip from '@/components/tooltips/info-tooltip'
import { Button } from '@/components/ui/button'
import {
    Popover,
    PopoverContent,
    PopoverTrigger,
} from '@/components/ui/popover'
import { Separator } from '@/components/ui/separator'

import { useModalState } from '@/hooks/use-modal-state'

import { IBaseProps, IClassProps, TEntityId } from '@/types'

type OtherFundTagManagerSize = 'sm' | 'default' | 'lg'

const otherFundTagChipVariants = cva(
    'flex items-center gap-x-1 rounded border bg-accent/50 text-accent-foreground',
    {
        variants: {
            size: {
                sm: 'px-2 py-1 text-xs',
                default: 'px-2 py-px text-sm',
                lg: 'px-3 py-1.5 text-base',
            },
        },
        defaultVariants: {
            size: 'default',
        },
    }
)

type OtherFundTagChipProps = {
    tag: IOtherFundTag
    onRemove?: () => void
    size?: OtherFundTagManagerSize
} & VariantProps<typeof otherFundTagChipVariants>

export const OtherFundTagChip = ({
    tag,
    onRemove,
    size = 'default',
}: OtherFundTagChipProps) => {
    const { onOpen } = useConfirmModalStore()

    const deleteMutation = useDeleteOtherFundTagById({
        options: { onSuccess: onRemove },
    })

    return (
        <InfoTooltip
            content={
                <p className="text-pretty text-foreground max-w-[400px]">
                    {tag.description}

                    {tag.created_at && (
                        <span className="text-[12px] block mt-2 text-muted-foreground">
                            {toReadableDateTime(tag.created_at)} (
                            {dateAgo(tag.created_at)})
                        </span>
                    )}
                </p>
            }
            delayDuration={700}
        >
            <div className={otherFundTagChipVariants({ size })}>
                {tag.icon && (
                    <RenderIcon
                        icon={tag.icon as TIcon}
                        style={{ color: tag.color }}
                    />
                )}
                <span>{tag.name}</span>
                {onRemove && (
                    <Button
                        className="size-fit cursor-pointer text-xs hover:text-red-600 disabled:opacity-50"
                        onClick={() =>
                            onOpen({
                                title: 'Remove Tag',
                                description:
                                    'Are you sure to remove this tag from this fund?',
                                confirmString: 'Remove',
                                onConfirm: () =>
                                    toast.promise(
                                        deleteMutation.mutateAsync(tag.id),
                                        {
                                            loading: 'Deleting tag...',
                                            success: 'Tag deleted',
                                            error: (error) =>
                                                serverRequestErrExtractor({
                                                    error,
                                                }),
                                        }
                                    ),
                            })
                        }
                        size="icon"
                        type="button"
                        variant="ghost"
                    >
                        <XIcon className="size-4" />
                    </Button>
                )}
            </div>
        </InfoTooltip>
    )
}

export function OtherFundTagsManager({
    readOnly,
    className,
    otherFundId,
    defaultTags,
    onSuccess,
    size = 'default',
}: {
    readOnly?: boolean
    otherFundId: TEntityId
    defaultTags?: IOtherFundTag[]
    onSuccess?: () => void
    size?: OtherFundTagManagerSize
} & IClassProps) {
    const tagPickerModal = useModalState()

    const {
        data: fundTags = [],
        isPending,
        refetch,
    } = useGetAllOtherFundTag({
        mode: 'other-fund',
        otherFundId,
        options: { initialData: defaultTags, retry: 0 },
    })

    const createTagMutation = useCreateOtherFundTag({
        options: {
            onSuccess:
                onSuccess ??
                (() => {
                    refetch()
                }),
        },
    })

    return (
        <div className={cn('space-y-2', className)}>
            <TagTemplatePicker
                modalState={tagPickerModal}
                onSelect={({ color, name, description, icon }) => {
                    toast.promise(
                        createTagMutation.mutateAsync({
                            name,
                            icon,
                            color,
                            description,
                            other_fund_id: otherFundId,
                        }),
                        {
                            loading: 'Adding tag...',
                            success: 'Tag Added',
                            error: (error) =>
                                serverRequestErrExtractor({ error }),
                        }
                    )
                }}
                triggerClassName="hidden"
            />
            <div className="w-full space-y-1">
                <div className="flex justify-between">
                    <p className="text-sm">Fund Tags</p>
                    {isPending && (
                        <LoadingSpinner className="size-3 mt-2 text-muted-foreground/70 inline" />
                    )}
                </div>
                <p className="text-xs text-muted-foreground">
                    Tags provide a quick context about this fund record.
                </p>
            </div>
            <Separator />
            <div className="flex gap-1.5 flex-wrap ">
                {!readOnly && (
                    <Button
                        className="border-dashed rounded size-fit! py-1 px-1! text-xs"
                        onClick={() => tagPickerModal.onOpenChange(true)}
                        type="button"
                        variant="outline"
                    >
                        <PlusIcon className="inline text-accent-foreground" />{' '}
                        Add Tag
                    </Button>
                )}
                {fundTags?.map((tag) => (
                    <OtherFundTagChip
                        key={tag.id}
                        onRemove={readOnly ? undefined : refetch}
                        size={size}
                        tag={tag}
                    />
                ))}
            </div>
            <p className="text-xs block text-muted-foreground">
                {fundTags.length} tag{fundTags.length !== 1 && 's'}
            </p>
        </div>
    )
}

export const OtherFundTagsManagerPopover = ({
    otherFundId,
    defaultTags,
    size = 'default',
    readOnly,
    className,
    children,
    onSuccess,
}: {
    otherFundId: TEntityId
    defaultTags?: IOtherFundTag[]
    size?: OtherFundTagManagerSize
    readOnly?: boolean
    onSuccess?: () => void
} & IBaseProps & { children?: ReactNode }) => {
    const { data: fundTags = [], isPending } = useGetAllOtherFundTag({
        mode: 'other-fund',
        otherFundId,
        options: {
            initialData: defaultTags,
            retry: 0,
            enabled: !!otherFundId,
        },
    })

    const tagCount = useMemo(() => fundTags.length, [fundTags])

    return (
        <Popover modal>
            <PopoverTrigger asChild>
                {children ? (
                    children
                ) : (
                    <Button
                        className={cn(
                            'size-fit p-0! border-accent rounded-full py-0.5! px-1.5!',
                            className
                        )}
                        size="sm"
                        type="button"
                        variant="outline"
                    >
                        <TagIcon />{' '}
                        <span>{isPending ? '...' : tagCount} Tags</span>
                    </Button>
                )}
            </PopoverTrigger>
            <PopoverContent className="w-auto min-w-[220px] rounded-xl max-w-[340px]">
                <OtherFundTagsManager
                    defaultTags={defaultTags}
                    onSuccess={onSuccess}
                    otherFundId={otherFundId}
                    readOnly={readOnly}
                    size={size}
                />
            </PopoverContent>
        </Popover>
    )
}
